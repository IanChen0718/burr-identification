...
class PdGeom():
    def __init__(self, df):
        "pc is an np array"
        self.df = df
        self.pc = df[["X", "Y", "Z"]].to_numpy() ## Point Cloud
        self.planarity = None
        self.n = self.pc.shape[0] ## number of point cloud
        self.n_neighbors = None
        self._div = 4
    def estimate(self, neighbors):
        ...
        _, idxNN_all_qp = kdtree.query(query_points, k=self.n_neighbors, p=2, workers=-1) # return distance and index 
        idxNN_all_qp_index = np.array(self.df.index)[idxNN_all_qp]
        ...
        self.df['Planarity'] = self.planarity.tolist()
        self.df['Neighbors'] = idxNN_all_qp_index.tolist()
        # self.df['PC1'] = pc1.tolist()
        # self.df['PC2'] = pc2.tolist()
        # self.df['Normal'] = normals.tolist()
        self.df['PC1X'] = pc1[:, 0].tolist()
        self.df['PC1Y'] = pc1[:, 1].tolist()
        self.df['PC1Z'] = pc1[:, 2].tolist()
        self.df['PC2X'] = pc2[:, 0].tolist()
        self.df['PC2Y'] = pc2[:, 1].tolist()
        self.df['PC2Z'] = pc2[:, 2].tolist()
        self.df['NormalX'] = normals[:, 0].tolist()
        self.df['NormalY'] = normals[:, 1].tolist()
        self.df['NormalZ'] = normals[:, 2].tolist()
        # return self.df
    
    def planarize(self, plan_threshold):
        df_plan = copy.deepcopy(self.df)
        df_corr = copy.deepcopy(self.df)
        df_plan = df_plan.drop(df_plan[df_plan["Planarity"] < plan_threshold].index)
        df_corr = df_corr.drop(df_corr[df_corr["Planarity"] >= plan_threshold].index)
        return df_plan, df_corr        
